namespace Less3.Database.SqlServer.Queries
{
    ...
    internal static class CredentialQueries
    {
        ...
        {
            return "SELECT TOP 1 * FROM credentials WHERE id = '" + Sanitizer.SanitizeString(id) + "';";
        }

        internal static string SelectById(string tenantId, string id)
        {
            return "SELECT TOP 1 * FROM credentials WHERE tenant_id = '" + Sanitizer.SanitizeString(tenantId) + "' AND id = '" + Sanitizer.SanitizeString(id) + "';";
        }
        ...
        {
            return "SELECT TOP 1 * FROM credentials WHERE accesskey = '" + Sanitizer.SanitizeString(accessKey) + "';";
        }
