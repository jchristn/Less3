namespace Less3.Database.SqlServer.Queries
{
    ...
    internal static class UploadQueries
    {
        ...
        {
            return "SELECT TOP 1 * FROM uploads WHERE id = '" + Sanitizer.SanitizeString(id) + "';";
        }

        internal static string SelectById(string tenantId, string id)
        {
            return "SELECT TOP 1 * FROM uploads WHERE tenant_id = '" + Sanitizer.SanitizeString(tenantId) + "' AND id = '" + Sanitizer.SanitizeString(id) + "';";
        }
