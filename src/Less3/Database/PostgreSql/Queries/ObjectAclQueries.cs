namespace Less3.Database.PostgreSql.Queries
{
    ...
    internal static class ObjectAclQueries
    {
        ...
        {
            return "SELECT * FROM objectacls WHERE object_id = '" + Sanitizer.SanitizeString(objectId)
        ...
        {
            return "SELECT COUNT(*) AS cnt FROM objectacls WHERE user_id = '" + Sanitizer.SanitizeString(userId)
                + "' AND object_id = '" + Sanitizer.SanitizeString(objectId)
                + "' AND bucket_id = '" + Sanitizer.SanitizeString(bucketId) + "';";
        }
        ...
        {
            return "DELETE FROM objectacls WHERE object_id = '" + Sanitizer.SanitizeString(objectId)
                + "' AND bucket_id = '" + Sanitizer.SanitizeString(bucketId) + "';";
        }
    }
}
