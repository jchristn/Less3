...
- `PostgresLockManager`: the in-database provider (in-process authority is Postgres). Each request is appended to a per-key FIFO queue (`less3_lock_queue`); the grant decision runs inside the `less3_lock_try_grant` PL/pgSQL function under a per-key `pg_advisory_xact_lock`, so exactly one node decides at a time. Lease expiry uses the DB clock; an expired lease/waiter is reclaimed; each grant bumps a per-key monotonic fencing token (`less3_lock_fence`) that a mutation re-checks before commit.
- `ClutchLockManager`: optional. Connects to a Clutch server over its native **WebSocket** lock protocol (`/v1.0/lock/connect`, `x-clutch-access-key` header) with one persistent connection per node — so every lock the node holds is auto-released by Clutch when the socket closes (a crash can't strand a lock until its lease lapses). No NuGet SDK dependency (raw `ClientWebSocket`). Clutch performs the fair read/write/delete drain server-side and shares the same Postgres via BYOD, so the DB stays authoritative. Alpha, but it is the **default lock provider in the Docker stack** (`Docker/system.node.json` sets `LockProvider: Clutch`) so the bundled Clutch server, its dashboard, and the dashboard's "Manage Locks" action are live out of the box; the connection is established lazily on a node's first lock.
- Selection is driven by `ClusterSettings.LockProvider` (`Local` | `Postgres` | `Clutch`). The Docker default is `Clutch`; switch to `Postgres` for in-database locking with no extra service. Both preserve the same integrity guarantees.
