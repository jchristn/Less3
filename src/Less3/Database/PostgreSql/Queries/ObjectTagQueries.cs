namespace Less3.Database.PostgreSql.Queries
{
    ...
    internal static class ObjectTagQueries
    {
        ...
        {
            return "SELECT * FROM objecttags WHERE object_id = '" + Sanitizer.SanitizeString(objectId)
        ...
        {
            return "DELETE FROM objecttags WHERE object_id = '" + Sanitizer.SanitizeString(objectId)
                + "' AND bucket_id = '" + Sanitizer.SanitizeString(bucketId) + "';";
        }
