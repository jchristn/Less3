namespace Less3.Database.PostgreSql.Queries
{
    ...
    internal static class ObjectAclQueries
    {
        ...
        {
            return "SELECT * FROM objectacls WHERE objectguid = '" + Sanitizer.SanitizeString(objectGuid)
        ...
        {
            return "SELECT COUNT(*) AS cnt FROM objectacls WHERE usergroup = '" + Sanitizer.SanitizeString(groupName)
                + "' AND objectguid = '" + Sanitizer.SanitizeString(objectGuid)
                + "' AND bucketguid = '" + Sanitizer.SanitizeString(bucketGuid) + "';";
        }

        internal static string ExistsByUserGuid(string userGuid, string objectGuid, string bucketGuid)
        {
            return "SELECT COUNT(*) AS cnt FROM objectacls WHERE userguid = '" + Sanitizer.SanitizeString(userGuid)
                + "' AND objectguid = '" + Sanitizer.SanitizeString(objectGuid)
                + "' AND bucketguid = '" + Sanitizer.SanitizeString(bucketGuid) + "';";
        }
        ...
        {
            return "DELETE FROM objectacls WHERE objectguid = '" + Sanitizer.SanitizeString(objectGuid)
                + "' AND bucketguid = '" + Sanitizer.SanitizeString(bucketGuid) + "';";
        }
    }
}
