namespace Less3.Database.SqlServer.Queries
{
    ...
    internal static class UserQueries
    {
        ...
        {
            return "SELECT TOP 1 * FROM users WHERE id = '" + Sanitizer.SanitizeString(id) + "';";
        }

        internal static string SelectById(string tenantId, string id)
        {
            return "SELECT TOP 1 * FROM users WHERE tenant_id = '" + Sanitizer.SanitizeString(tenantId) + "' AND id = '" + Sanitizer.SanitizeString(id) + "';";
        }

        internal static string SelectByName(string name)
        {
            return "SELECT TOP 1 * FROM users WHERE name = '" + Sanitizer.SanitizeString(name) + "';";
        }

        internal static string SelectByName(string tenantId, string name)
        {
            return "SELECT TOP 1 * FROM users WHERE tenant_id = '" + Sanitizer.SanitizeString(tenantId) + "' AND name = '" + Sanitizer.SanitizeString(name) + "';";
        }

        internal static string SelectByEmail(string email)
        {
            return "SELECT TOP 1 * FROM users WHERE email = '" + Sanitizer.SanitizeString(email) + "';";
        }

        internal static string SelectByEmail(string tenantId, string email)
        {
            return "SELECT TOP 1 * FROM users WHERE tenant_id = '" + Sanitizer.SanitizeString(tenantId) + "' AND email = '" + Sanitizer.SanitizeString(email) + "';";
        }
