...
- Targeted `net10.0`.
- Added a pluggable distributed lock manager (`ILockManager`) with `Local` (in-process, single-node), `Postgres` (cluster default; the database is the lock authority, acquisition is one serialized transaction per key), and optional `Clutch` (alpha, over Clutch's native WebSocket lock protocol with one persistent connection per node — so a node crash auto-releases every lock it held instead of stranding it until the lease lapses — sharing the same Postgres via bring-your-own-database) providers.
