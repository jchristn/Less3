namespace Less3.Database.SqlServer.Queries
{
    ...
    internal static class BucketQueries
    {
        ...
        {
            return "SELECT TOP 1 * FROM buckets WHERE id = '" + Sanitizer.SanitizeString(id) + "';";
        }

        internal static string SelectById(string tenantId, string id)
        {
            return "SELECT TOP 1 * FROM buckets WHERE tenant_id = '" + Sanitizer.SanitizeString(tenantId) + "' AND id = '" + Sanitizer.SanitizeString(id) + "';";
        }

        internal static string SelectByName(string name)
        {
            return "SELECT TOP 1 * FROM buckets WHERE name = '" + Sanitizer.SanitizeString(name) + "';";
        }

        internal static string SelectByName(string tenantId, string name)
        {
            return "SELECT TOP 1 * FROM buckets WHERE tenant_id = '" + Sanitizer.SanitizeString(tenantId) + "' AND name = '" + Sanitizer.SanitizeString(name) + "';";
        }
