namespace Less3.Database.PostgreSql.Queries
{
    ...
    internal static class ObjectTagQueries
    {
        ...
        {
            return "SELECT * FROM objecttags WHERE objectguid = '" + Sanitizer.SanitizeString(objectGuid)
        ...
        {
            return "DELETE FROM objecttags WHERE objectguid = '" + Sanitizer.SanitizeString(objectGuid)
                + "' AND bucketguid = '" + Sanitizer.SanitizeString(bucketGuid) + "';";
        }
    }
}
